...
def total_salary(path: str) -> tuple:
    try:
        with Path(path).open('r', encoding="utf-8") as fh:
            list_of_salary = [int(el.split(",")[1]) for el in fh.readlines()]
            return sum(list_of_salary), sum(list_of_salary) // len(list_of_salary)
...
if __name__ == "__main__":
    while True:
        ...
        if user_input == "exit":
            ...
            total, average = total_salary(user_input)
            print(f"Загальна сума заробітної плати: {total}, Середня заробітна плата: {average}")
